...
class RequestHandler:
    ...
    def say_weather_current(self,place):
        ...
        if self.get(url) == True:
            ...
            self.parent.say(f"{data["current"]["temp_f"]}  degrees fahrenheit")
            self.parent.say(f"{data["current"]["temp_c"]}  degrees celcius")
            self.parent.say(f" wind speed is {data["wind_mph"]} miles per hour")
        else:
           self.issue()
